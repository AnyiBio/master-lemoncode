const students: Array<Student> = [
  ...
  const summarizeClassRoom = (studentList: Array<Student>): Array<SummarizeClassRoom> => {
    return studentList.map((el => {
        const highestCalification: number = +Math.max(...el.califications).toPrecision(3);
        const total: number = el.califications.reduce((acc, c) => acc + c, 0);
        const averageCalifications: number = total / el.califications.length;
        return {
            name: el.name,
            highestCalification: highestCalification,
            averageCalifications: averageCalifications.toPrecision(3).toString(),
        }
    }))
  };
